...
fex_a1 = tf.get_varable('alpha1', f_cnn_h1.get_shape()[-1], initializer=tf.constant_initializer(0.25))    
...
lstm_multi = tf.nn.rnn_cell.MultiRNNCell([lstm_en_cell, lstm_de_cell])
output, state = tf.nn.dynamic_rnn(lstm_multi, feature, dtype=tf.float32)
output = tf.reshape(output, [tf.shape(x)[0], 2000, 1, 256])

f_dcnn_h1 = deconv(output, frec_W1, [tf.shape(x)[0], 4000, 1, 1])
